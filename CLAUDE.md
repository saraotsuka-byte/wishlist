...
- **数値入力欄(在庫・発注点・標準購入数・買い物リストの購入数)はReact state上で文字列として保持する**。`value={stock} onChange={(e) => setStock(Math.max(0, Number(e.target.value)))}` のように毎回数値へ丸めて即座に反映すると、入力欄を空にした瞬間に `Number('')` が `0` と評価されて即座に「0」へ戻ってしまい、ユーザーが初期値を消して入力し直せないという不具合になる（実際に発生し修正済み）。数値への変換・下限のclampは送信時（`ItemForm.tsx` の `handleSubmit`）またはフォーカスが外れた時（`ShoppingList.tsx` の買い物リスト行の購入数、`onBlur` でコミット）にのみ行い、入力中は生の文字列をそのまま表示する。
